...
This repository has a local context layer (MrFoX-MeM) available over MCP that acts as durable, cross-session project memory.

- At the start of every task, call the MCP tool `get_relevant_context({ prompt, budget_tokens })`, passing a short description of the task as `prompt` (for example `budget_tokens: 2000`). Read the returned memory before proposing changes.
